...
use function RRZE\Glossary\Config\getOptionName;
use function RRZE\Glossary\Config\getMenuSettings;
use function RRZE\Glossary\Config\getHelpTab;
use function RRZE\Glossary\Config\getSections;
use function RRZE\Glossary\Config\getFields;
...
class Settings {
    ...
    protected function setFields() {
        ...
        if ( isset($_GET['page']) && $_GET['page'] == 'rrze-glossary' && isset($_GET['current-tab']) && $_GET['current-tab'] == 'glossarysync' ){
            // Add Sync fields for each domain
            $this->settingsFields['glossarysync'] = $this->setSettingsDomains();
        }
    }
    ...
    public function setSettingsDomains(){
        ...
        foreach ( $aDomains as $shortname => $url ){
            ...
            foreach ( $this->settingsFields['glossarysync'] as $field ){
                ...
                switch ( $field['name'] ){
                    ...
                    case 'categories':
                        if ( !$aCategories ){
                            $field['options'][''] = __( 'no category with source = "website" found', 'rrze-glossary' );
                        }
